...
export function createComicSeriesService(Model: ComicSeriesModel): ComicSeriesService {
  async function updateSeries(series: string, update: any): Promise<ComicSeries> {
  ...
  return {
    ...
    getByIdWithComicBooksUrl: async id => Model.findById(id).populate('comicBooks', 'url').exec(),
    insert: async (comicSeries, comicBook) => updateSeries(comicSeries, { $push: { comicBooks: comicBook } }),
  }
}