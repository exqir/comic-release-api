...
const PullListMutation = {
  ...
  pullSeries: async (root, { owner, publisher, seriesUrl }) => {
    const existingSeries = await Series.findOne({ publisher: publisher._id, url: seriesUrl }).exec()
    ...
    const { _id } = await new Series({...series, publisher: publisher._id}).save()
...
async function pushSeriesToPullList(owner, series) {
  //returns null if pullList is not updated -> has to be handled as a PullList has to be returned
  const updatedPulllist = await PullList.findOneAndUpdate({ owner: owner, list: {$ne: series} }, { $push: {list: series} }, { new: true })  
  return updatedPulllist || await getPullListByOwner(owner)  
} 
