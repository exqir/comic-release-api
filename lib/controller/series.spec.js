...
describe('series controller tests', () => {
  ...
  beforeEach(() => {
    ...
    td.when(publisherAdapter.getPublisher('image')).thenResolve(imageConfig)
  })

  describe('create new series', () => {
    beforeEach(() => {
      td.when(scraper.scrap('series', imageConfig, '/comics/series/test')).thenResolve(series)
      td.when(newSeries(enhancedSeries)).thenResolve({...enhancedSeries, _id: '1234'})
    })
    ...
    it('should return falsy value when failed to scrap', async () => {
      td.when(scraper.scrap('series', imageConfig, '/comics/series/test')).thenResolve({})
    ...
    it('should return falsy value when failed to save', async () => {
      td.when(newSeries(enhancedSeries)).thenResolve(null)
      const id = await seriesModule.createNewSeries('image', '/comic/series/test')
  ...
  describe('post save hook for new series', () => {
    it('should call update functions with correct parameters', async () => {
      await seriesModule.postSave({...enhancedSeries, _id: '1234'}, () => null)
